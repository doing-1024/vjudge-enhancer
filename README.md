...
- **0.5.1** — 搜索面板新增「收藏夹内」模式，可即时过滤本地收藏题目。
- **0.5.0** — 统一右侧动作按钮栏（可折叠）、单窗口管理、点外关闭；配色贴合 vjudge + 动效；收藏星标弹跳等。

## 🆕 更新日志

- **0.5.3** — 修复题目页底部出现重复无图标按钮的问题。
- **0.5.2** — 修复 vjudge SPA body 替换导致的重复按钮问题。
- **0.5.1** — 搜索面板新增「收藏夹内」模式，可即时过滤本地收藏题目。
